...
public class OperandFetch {
	...
	public void performOF()
	{
		...
		// }
		System.out.println("Is OF enabled?");
		...
		if(IF_OF_Latch.isOF_enable())
		{
			...
			System.out.println("rd value in next stages:");
			// int x = EX_MA_Latch.getRd();
			...
			// }
			// else if(Clock.getCurrentTime()  < 4)
			// {
			// 	EX_MA_Latch = new EX_MA_LatchType();
			// 	MA_RW_Latch = new MA_RW_LatchType();
			// 	MA_RW_Latch.setRd(40);
			// }
			if(OF_EX_Latch.isEX_enable()==false)
			{
				OF_EX_Latch.setEX_rd(40);
				OF_EX_Latch.setMA_rd(40);
			...
			if(OF_EX_Latch.getEX_rd() == rs1_val || OF_EX_Latch.getEX_rd() == rs2_val || OF_EX_Latch.getMA_rd() == rs1_val || OF_EX_Latch.getMA_rd() == rs2_val || OF_EX_Latch.getRW_rd() == rs1_val || OF_EX_Latch.getRW_rd() == rs2_val || OF_EX_Latch.getEX_rd()==31 || OF_EX_Latch.getMA_rd()==31 || OF_EX_Latch.getRW_rd()==31){
				if(rs1_val != 0 && rs1_val != 1 && rs1_val != 2 && rs2_val != 0 && rs2_val != 1 && rs2_val != 2){
					OF_EX_Latch.setInstruction(0);
					IF_OF_Latch.set_Stall(true);
				}
			}
			// if(OF_EX_Latch.getRd() == rs1_val || OF_EX_Latch.getRd() == rs2_val || EX_MA_Latch.getRd() == rs1_val || EX_MA_Latch.getRd() == rs2_val || MA_RW_Latch.getRd() == rs1_val || MA_RW_Latch.getRd() == rs2_val || MA_RW_Latch.getRd()==31){
			// 	if(rs1_val != 0 && rs1_val != 1 && rs1_val != 2 && rs2_val != 0 && rs2_val != 1 && rs2_val != 2){
			// 		OF_EX_Latch.setInstruction(0);
			// 		IF_OF_Latch.set_Stall(true);
			// 	}
			// }
				...
				// IF_EnableLatchType IF_Enable = new IF_EnableLatchType();
				// IF_OF_Latch.
			...
			System.out.println("Is inst to be stalled?");
			System.out.println(IF_OF_Latch.is_Stall());
			OF_EX_Latch.setEX_enable(true);
